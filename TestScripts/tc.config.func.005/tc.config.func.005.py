...
TESTCASE_ID = "Tc.config.func.005"
TESTCASE_PRI = "M"
...
if __name__ == '__main__':
    """When config different TIME zone,The node can change to the TIME zone."""
    #####################<prerequisites>########################
    ...
    #####################<Test procedure>########################
    # TODO:currently,timezone on device cannot be modified
    ssh_dp_proc.sendline("python update_config.py %s \"%s\"" %
                         (conf['nodes']['1st']['name'], str({"timeZone": "MIT"})))
    wait_server(conf, ssh_dp_proc, 'dp')
    if not(ssh_dp_proc.before.find('failure') == -1):
        ...
        raise Exception('abortion failure')
    time.sleep(120)
    ssh_vpn_proc.sendline("python check_time.py '%s' '%s'" % (conf['nodes']['1st']['name'], 'MIT'))
    wait_server(conf, ssh_vpn_proc, 'vpn')

    ssh_dp_proc.sendline("python update_config.py %s \"%s\"" %
                         (conf['nodes']['1st']['name'], str({"timeZone": "UTC"})))
    wait_server(conf, ssh_dp_proc, 'dp')
    if not(ssh_dp_proc.before.find('failure') == -1):
        print('update config failure: %s' % (ssh_dp_proc.before))
        raise Exception('update config failure')
    ...
    #####################<check postcondition>########################
    ssh_vpn_proc.sendline("python check_time.py '%s' '%s'" % (conf['nodes']['1st']['name'], 'UTC'))
    wait_server(conf, ssh_vpn_proc, 'vpn')
    if not(ssh_vpn_proc.before.find('failure') == -1):
