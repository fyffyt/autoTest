...
TESTCASE_ID = "Tc.config.func.010"
TESTCASE_PRI = "H"
...
if __name__ == '__main__':
    # TODO need some discuss about how to config wifi control module
    """When a node's config file has updated , wifi control module will adjust its behavior mode properly"""
    #####################<prerequisites>########################
    ...
    # 2nd para for check_module_health: 'config','log','wificontrol'
    ssh_dp_proc.sendline("python check_module_health.py '%s' \"%s\""
                         % (conf['nodes']['1st']['name'], str(['log', 'config', 'wificontrol'])))
    wait_server(conf, ssh_dp_proc, 'dp')
