...
TESTCASE_ID = "Tc.vworker.func.001"
TESTCASE_PRI = "H"
...
if __name__ == '__main__':
    """Video module in each nodes can start up and\
            run correctly in 2 minutes after the node start up."""
    ...
    ssh_dp_proc.sendline("python check_module_health.py '%s' \"%s\"" %
                         (conf['nodes']['1st']['name'], str(['config', 'log', 'video'])))
    wait_server(conf, ssh_dp_proc, 'dp')
