...
TESTCASE_ID = "Tc.upload.func.004"
TESTCASE_PRI = "L"
...
if __name__ == '__main__':
    """Upload module can upload video files which have correct format."""
    ...
    wait_server(conf, ssh_vpn_proc, 'node')
    # TODO what's the upper size limit of video files
    ssh_vpn_proc.sendline(
        "busybox dd if=/dev/zero bs=1M count=48 of=%s" %
        (os.path.join(
            conf['dev_video_dir'],
            nameString)))
    wait_server(conf, ssh_vpn_proc, 'node')
