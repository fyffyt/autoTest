...
TESTCASE_ID = "Tc.upload.func.006"
TESTCASE_PRI = "M"
...
if __name__ == '__main__':
    """Upload module have r&w authority to all data files."""
    ...
    # clear data dir, generate file, chmod 444
    ...
    wait_server(conf, ssh_vpn_proc, 'node')
    ssh_vpn_proc.sendline("chmod 444 %s/*" % (conf['dev_video_dir']))
    wait_server(conf, ssh_vpn_proc, 'node')
    ssh_vpn_proc.sendline("chmod 444 %s/*" % (conf['dev_wifi_dir']))
    wait_server(conf, ssh_vpn_proc, 'node')
    ssh_vpn_proc.sendline("chmod 444 %s/*" % (conf['dev_blob_dir']))
    wait_server(conf, ssh_vpn_proc, 'node')
