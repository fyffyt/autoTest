...
TESTCASE_ID = "Tc.config.reli.009"
TESTCASE_PRI = "M"
...
if __name__ == '__main__':
    """When video config arguments are configured to extreme values, video module can still work"""
    ...
    #####################<Test procedure>########################
    # config video configuration to extreme value.then check if the video module can still work
    ssh_dp_proc.sendline("python update_config.py %s \"%s\"" %
                         (conf['nodes']['1st']['name'], str({"capture-height": "1080", "capture-width": "1920"})))
    wait_server(conf, ssh_dp_proc, 'dp')
    if not(ssh_dp_proc.before.find('failure') == -1):
    ...
    wait_server(conf, ssh_dp_proc, 'dp')
    if not(ssh_dp_proc.before.find('failure') == -1):
        print('test procedure failure: %s' % (ssh_dp_proc))
        raise Exception('check_module_health failure')
    elif not(ssh_dp_proc.before.find('success') == -1):
        print('module health ok.')
    else:
    ...
    ssh_dp_proc.sendline("python update_config.py %s \"%s\"" %
                         (conf['nodes']['1st']['name'], str({"capture-height": "0", "capture-width": "0"})))
    wait_server(conf, ssh_dp_proc, 'dp')
    if not(ssh_dp_proc.before.find('failure') == -1):
        print('update config failure: %s' % (ssh_dp_proc.before))
        raise Exception('update config failure')
