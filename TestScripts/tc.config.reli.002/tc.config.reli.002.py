...
TESTCASE_ID = "Tc.config.reli.002"
TESTCASE_PRI = "L"
...
if __name__ == '__main__':
    """Config module need to check remote config file format
    after DP server update it with wrong config format"""
    ...
    wait_server(conf, ssh_vpn_proc, 'vpn')
    if not(ssh_vpn_proc.before.find('failure') == -1):
        print('Test failure: %s' % (ssh_vpn_proc.before))
        ...
        print('abortion failure: %s' % (ssh_vpn_proc.before))
        raise Exception('abortion failure')
    # modify the config file on server with wrong format,
    # expect that the relative device won't change its local configuration
    ssh_dp_proc.sendline("echo 'error format test' > %s.json" %
