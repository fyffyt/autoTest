...
TESTCASE_ID = "Tc.upgrade.reli.005"
TESTCASE_PRI = "M"
...
if __name__ == '__main__':
    """check ci TestTemplate after upgrading, this scripts should run in formal update procedure, recommend to execute
    it manually"""
    ...
    # run ci TestTemplate
    pexpect.run("bash -c 'python ../../test_main.py ci &'")
    #####################<check postcondition>########################
    ssh_vpn_proc.sendline("python check_node_health.py '%s'" % (conf['nodes']['1st']['name']))
    wait_server(conf, ssh_vpn_proc, 'vpn')
    if not(ssh_vpn_proc.before.find('failure') == -1):
