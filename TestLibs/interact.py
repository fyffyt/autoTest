...
def set_vworker_mode(_conf,
    ...
    if not(ssh_dpsvr_proc.before.find('failure') == -1):
        ...
        print('abortion failure: %s' % (ssh_dpsvr_proc.before))
...
def chk_vworker_mode(_conf,
    ...
    while(_counter1 > 0):
        ...
        if not(ssh_dpsvr_proc.before.find('success') == -1):
            ...
            _counter1 -= 1
            print('try %s times.' % (str(20 - _counter1)))
            time.sleep(30)
    if(0 == _counter1):
    ...
    if (num1 >= num2):
        print('chk_vworker_mode failure')
        raise Exception('chk_vworker_mode failure')
