...
class Device(object):
    ...
    def __init__(self, name=None):
        ...
        if name is None:
            self.PlacementID = self.mContext.getTestDevice()
        else:
            self.PlacementID = name
        #storageType = "google" or "azure"
        ...
        #clear piled data files on test device
        self.runCommand('rm -rf /sdcard/FileInfo/*')
        self.restartFS()
...
#       self.executor = pexpect.spawn('bash')
    ...
    def runCommand(self, cmd, retry = 3):
        ...
        regex4 = re.compile(r'(?im)^.*(sh|scp):.*connection closed.*$')#sh: connectio aborted
        ...
        elif not regex4.search(self.executor.before) is None:
            if _retry >= 1:
                time.sleep(30)
                return self.runCommand(cmd, _retry - 1)
            else:
    ...
    def rebootDevice(self):
        ...
        if not (_uptimeAfter < _uptimeBefore or
                _uptimeBefore < "0 days, 00:02:00"):
    ...
    def getCpuUsage(self):
        ...
        """
        REGEX_FLOAT = r"(?<=\W)\d{1,8}\.?\d{0,16}(?=\W)"
        REGEX_INT = r"(?<=\D)\d+(?=\D)"
        ...
        ret1 = self.runCommand('su -c \'dumpsys cpuinfo\'')
        ret2 = self.runCommand('su -c \'iostat\'')
...
#               % (REGEX_FLOAT, REGEX_FLOAT))
        regexTotalUsage = re.compile(r'(?im)^\s*(%s)\s*(%s)\s*(%s)\s*(%s)\s*(%s)\s*(%s)\s*$'
                % (REGEX_FLOAT, REGEX_FLOAT, REGEX_FLOAT, REGEX_FLOAT, REGEX_FLOAT, REGEX_FLOAT)
                )

        m1 = regexLoad.search(ret1)
        m2 = regexFSUsage.search(ret1)
        m3 = regexTotalUsage.search(ret2)
        if m1 is None or m2 is None or m3 is None:
            raise RuntimeError("extracting cpu usage info from \"%s\"\n and\n \"%s\" failed"
                    % (ret1, ret2)
                    )
            ...
            d['totalUsage'] = {'user':m3.group(1), 'nice':m3.group(2), 'system':m3.group(3),\
                    'iowait':m3.group(4), 'steal':m3.group(5), 'idle':m3.group(6)}
    ...
    def getDevCfgFromAS(self):
        ...
        if hostname.__contains__("ci-api"):
            ...
            container = "percolata-data"
        else:
            raise NotImplementedError("cannot get config from %s yet" % hostname)

        remoteFilePath = "as://" + container + '/config/%s.json' % self.PlacementID
    ...
    def sysmon(self, logger):
        """
        Monitor the system cpu, mem, sdcard usage.
        Relative threshold are 90%, 800M/900M, 5.3G/5.5G(for motoG devices).
        @param logger: TestLibs.bs_logger.BSLogger
        """

        if self.getCpuUsage()['totalUsage']['idle'] < "10.00":
            logger.w("available cpu time is lower than 10%")

        if self.getMemUsage()['freeRAM'] < "100000":
            logger.w("available mem space is less than 100MB")

        if self.getSdcardUsage()['free'] < "200":
            logger.w("available sdcard space is less than 200MB")
    ...
    def wait_server(self, timeout=60, retry=1):
        ...
        if 0 == switch:
            ...
            if retry > 0:
                self.executor.sendline("")
                self.wait_server(timeout, retry - 1)
            else:
